...
- 線上版（建議）：<https://sauloveling.github.io/Personal_Memo_Note/>
- 離線版：直接用瀏覽器打開 `index.html`，不用安裝任何東西

### 安裝成電腦／手機 App

本程式是 PWA，可安裝成獨立 App（有自己的圖示與視窗，沒有瀏覽器網址列），且支援離線開啟：

| 平台 | 安裝方式 |
|---|---|
| Windows（Chrome／Edge） | 開啟網址後，網址列右側會出現「安裝」圖示，或選單 →「安裝翻譯備忘簿」 |
| Mac（Chrome／Edge） | 同上；Safari 則用「檔案 → 加入 Dock」 |
| iPhone／iPad | Safari 開啟 → 分享 → 加入主畫面 |
| Android | Chrome 開啟 → 選單 → 安裝應用程式／加入主畫面 |

安裝後會照常自動更新（採網路優先策略，有網路時永遠載入最新版；沒網路時用上次成功載入的版本）。
...
- **文字格式**：編輯區上方工具列可設定粗體、斜體、底線、大小（小/一般/大）、文字顏色（預設色＋紅橘綠藍紫），以及清除格式。工具列會固定在上方，捲到長筆記的任何位置都能直接套用格式。格式會跟著同步；翻譯與搜尋一律使用純文字，不受格式影響。
  - 「預設色」是移除文字顏色（跟著淺色／深色主題走），不是寫死黑色，所以深色模式下不會變得看不見。
  - 「✕ 清除格式」會一併清掉從網頁貼上時帶進來的**底色、外框、字體**。有選取文字就只清該範圍，沒有選取則清整篇（可按 Ctrl+Z 復原）。
  - 從其他網頁貼上的內容會**自動過濾**，只保留粗體／斜體／底線／文字顏色，不會把底色和外框帶進來。
- **超連結**：筆記裡的網址（`http://`、`https://`、`www.` 開頭）會自動變成可點擊的連結，**點一下即可開啟**。工具列最右邊的 🔗 是開關：
  - 開啟（預設）：網址顯示成藍色連結，點連結會開啟網頁。
  - 關閉：網址變回純文字，方便你直接編輯連結內容。
  - 連結只是「顯示用」，翻譯與搜尋一律使用純文字網址，不受影響；儲存與同步的內容也不含多餘的連結標記。
...
- **行事曆**：按「📅 行事曆」開啟月曆檢視，點任一天即可寫當天的事項（可多筆），像紙本月曆一樣。有事項的日子會顯示內容、今天會標示，可切換月份。行事曆事項跟著同步，手機電腦共用。**當天的事項會在早上連同例行提醒一起通知你**（同一則、各自分區塊，同一項一天只通知一次）。
...
- **附件**：按「📎 附加檔案」可附上 PDF、Word、Excel、TXT 等檔案，之後隨時按「下載」取回。附件會跟著同步，手機電腦都拿得到。
  - 單一檔案上限 **1MB**（超過會擋下並說明原因）。檔案內容存在瀏覽器的 IndexedDB，**不佔用** localStorage 的 5MB 額度，所以附件再多也不會導致筆記存不進去。
  - 其他裝置剛加的附件，在本機同步下來前會顯示「同步中…」，同步完成即可下載。
  - 若需要放更大的檔案（掃描 PDF、大型報表），可改用「另開私密倉庫」的儲存方式，容量可到數十 MB，但需要重新產生一組有倉庫權限的金鑰。
- **需確認的提醒（閉環）**：新增或編輯提醒時勾選「需要我按完成確認」，這則提醒到期後會**每天持續通知**（顯示「第 N 天」），直到你確認為止 —— 這樣就算到期日剛好放假漏看，隔天還會再提醒，不會靜悄悄過期。確認方式：
  - 每則通知（TG／LINE／Email）都附一個「✅ 完成」連結，**點一下**就開啟 app 並標記完成、停止提醒。三個管道通用。
  - 或直接在 app 的「⏰ 提醒」裡按該則的「✅ 完成」。
  - 提醒清單會用標籤顯示狀態：`需確認`／`⚠️ 待確認`／`✅ 已完成`。
- **備份**：「備份全部」匯出全部筆記為 json（含圖片、**附件**與垃圾桶），「還原」可匯回（也相容舊版備份檔）；「匯出 txt」匯出目前這篇（純文字，不含圖片與附件）。

## 提醒設定

例行公事提醒（例如「每月 4 號申報歐洲 VAT」）由 GitHub Actions 每天早上自動檢查發送，電腦手機關機也照樣運作，完全免費。

⏰ **關於送達時間**：GitHub 的免費排程**不保證準時**，實測常延遲 1～3 小時，偶爾會整次跳過。因此排程刻意提早並安排多個時段（台北 06、07、08、09、11 點各試一次），實際送達約落在早上 6 點到中午之間。程式有「同一則當天只發一次」的防重複機制（`lastSent`），所以多跑幾次不會收到重複通知。若需要準時送達，就得改用付費排程或自架服務。
