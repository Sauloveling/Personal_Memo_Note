...
- **快速加入行事曆**：行事曆裡按「⚡ 快速加入」，可**貼上一整段文字**（自動解析每行的日期）或**丟一張截圖**（瀏覽器內 OCR 辨識，圖片不外傳）。程式列出偵測到的「日期＋事項」清單，勾選、微調後一次批次加入。日期支援 `8/20`、`8月22日`、`2026/9/1` 等格式；沒寫年份時，已過的日期會自動當成明年。OCR 首次使用會下載中文模型（約十幾 MB，之後快取）。
